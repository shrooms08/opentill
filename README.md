...
Merchant **payouts** remain simulated in the product, and the reason is now
settled at source level by Tachi: **the protocol has no ledger→L1 path for
receipts today** — `TxWithdraw` has no handler, and `TxLockForVault` only flips
a ledger-side `Locked` flag with no L1 activity ("a real gap, not a hidden
design choice"). The vault exit above moves a vault's own L1 deposit, which is
why it only works for self-deposited funds. Unblocked only by Tachi shipping
ledger→L1 offboarding. See [INTEGRATION.md](INTEGRATION.md) §5.
...
| **Payout & liquidity flow** | Cooperative withdrawal **and** unilateral exit (sweep to on-chain Bitcoin, no permission) — full lifecycle in mock mode; **proven for real on regtest via a Taurus vault** (exit `5bb2960b…d0c3`, co-signed refund `b78cdb62…f86b`). Receipts cannot reach it yet: the protocol has no ledger→L1 offboarding today (INTEGRATION.md §5). | Test suite `gate4.test.ts` (both lifecycles, exit lock); screen `04-payouts-exit`. |
...
- **Real settlement covers receiving; payouts are simulated because the protocol has no ledger→L1 path for receipts yet.** `ADAPTER_MODE=tachi` is live on Tachi regtest for invoices, detection, confirmation and refunds (e2e: payment `d8fb214e…146f`, refund `698e3128…bf71`). The vault path — `createVault → depositToVault → registerVault → unilateral exit` (`5bb2960b…d0c3`) and a 5-of-7 co-signed cooperative refund (`b78cdb62…f86b`) — ran for real, but it moves a vault's own L1 deposit. Tachi traced the candidates in source: `TxWithdraw` has no handler and `TxLockForVault` is an off-chain hold with no L1 activity, so nothing pays out on L1 against a receipt today — a protocol gap, not an SDK or implementation one. `initiatePayout` returns a `failed` payout saying so; `ADAPTER_MODE=mock` demonstrates the payout/exit UX. Details: **[INTEGRATION.md](INTEGRATION.md)** §5.
